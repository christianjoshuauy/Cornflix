...
export default function Poster({ movie, isLarge }) {
  const { backdrop_path, genre_ids, original_title, overview, poster_path, release_date, title, name, vote_average } = movie
  const movieTitle = original_title || title || name
  const genres = useGenreConversion(genre_ids)
  const posterImage = useMemo(() => `${BASE_IMG_URL}/${poster_path}`, [poster_path])
  const backdropImage = useMemo(() => `${BASE_IMG_URL}/${backdrop_path}`, [backdrop_path])
  return (
    <div className={`Poster ${isLarge && "Poster--big"}`}>
      ...
      <div className="Poster-info">
        <div className="Poster-info--iconswrp">
          <a
            className="Poster-info--icon icon--play"
          >
            <FaPlay />
          </a>
          {true
