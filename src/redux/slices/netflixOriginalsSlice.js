...
export const fetchNetflixOriginals = createAsyncThunk('netflixOriginals/fetchNetflixOriginals', async() => {
    const response = await axios.get(`discover/tv?api_key=${process.env.REACT_APP_TMDB_API_KEY}&language=en-US&sort_by=popularity.desc&page=1&timezone=America%2FNew_York&with_networks=213&include_null_first_air_dates=false`)
...
export const { selectAll: selectAllNetflixOriginals } = netflixAdapter.getSelectors(state => state.netflixOriginals)
