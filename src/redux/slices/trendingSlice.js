...
export const fetchTrending = createAsyncThunk('trending/fetchTrending', async() => {
    const response = await axios.get(`trending/all/day?api_key=${process.env.REACT_APP_TMDB_API_KEY}`)
...
export const { selectAll: selectAllTrending } = trendingAdapter.getSelectors(state => state.trending)
